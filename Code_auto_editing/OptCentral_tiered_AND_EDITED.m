...
% param.Bom = triu(ones(nz,nz),1).*randi([0,1],nz,nz); %param.Bom(i,I) = number of units of item I required to produce 1 unit of item i DIRECTLY (aka "B").
...
%
%
% param.R_z = 3*ones(nz,nloc); %param.R_z(i,k) = NUMBER OF TIME-STEPS REQUIRED TO PRODUCE PRODUCT i AT LOCATION k
% param.R_t = 4*ones(nloc, nloc); %param.R_t(k,L) = NUMBER OF TIME-STEPS REQUIRED TO TRANSPORT FROM LOCATION k TO LOCATION L
%
% param.s_y = 1*ones(nz,1); %param.s_y(i) denotes the number of "units" (of space, load, etc.) required to hold one unit of product/component i
%
% param.c_y = 250*ones(nloc,1); %param.c_y(k) = the storage capacity (in "units") at location k
% param.c_truck = 75; %param.c_truck is the number of "units" that a truck can fit
% %param.c_z = 10*ones(nz,nloc); %param.c_z(i,k) = capacity to produce product i at location k in one production cycle
% param.c_z = [250*randi([0,1],nz,numProdSites),zeros(nz,nloc-numProdSites)]; %nz X nloc
% %param.c_z = 10*ones(nz,nloc); %Extra easy dummy
% param.f_z0 = 5*ones(nz,nt,nloc); %param.f_z0(i,j,k)=fixed cost of starting production of product i at time-step j at location k
% param.f_z = .1*ones(nz,nt,nloc); %param.f_z(i,j,k) = PRICE OF PRODUCTION per unit of product i, at location k, at time-step j
% param.f_y = .1*ones(nt,nloc); %param.f_y(j,k) = PRICE OF HOLDING one "unit" at a given location, at a given time (possibly not so variable over time)
% param.f_t = 10*ones(nloc,nloc); %param.f_t(k,L) = price of sending one truck from location k to location L. If param.f_t(k,L) <0, then no truck will make that route. By convention, set to -1.
% dummy1= randi([0,1],nloc,nloc); %Experiment
% dummy2= randi([0,1],nloc,nloc); %Experiment
% param.f_t = param.f_t - 2*(dummy1 + dummy2 - dummy1.*dummy2); %Experiment - want lots of non-travellable routes.
% param.f_t = 5*param.f_t;
...
% %}
%
% param.mu_w = .01*ones(nz,nloc); %Sales backlog decay constants.
% param.perMileTruckCost = .2;
% param.hourlyTruckCost = .2;
% param.fixedTruckCost = 1.5;
%
...
% % wc= wc.*(param.c_z==0);%Arbitrary BUT this ensures that locations
...
%
% param.zc = (param.c_z >0); %param.zc(i,k) ==1 iff capacity to produce param.c_z(i,k) is > 0
%
% tc = ones(nloc,nloc); %tc(k,L) = 1 if trucking can happen between locations k and L, 0 otherwise.
% tc = tc - (param.f_t<0);


zNonProducers = ~any(param.c_z,1); %Locations that have NO production can be stripped of their z00 variable at all time-steps


zPrune = nnz(param.zc<1); %Number of pairs of product and location at which production will never happen.
    ...
    for k = 1:nloc
        ...
        if (wc(i,k) == -1) %Can BUY product i at location k
            for j = 1:nt
                ...
                LB(J(4,i,j,k,1)) = -param.c_y(k)/param.s_y(i); %Can buy only what can be held at location.
        ...
        elseif (wc(i,k)==1) %Can SELL product i at location k
            ...
            for j = 1:nt
                UB(J(4,i,j,k,1)) = 0;
                for T = 1:j
                    UB(J(4,i,j,k,1)) = UB(J(4,i,j,k,1))+ param.mu_w(i,k)^(j-T)*dem(i,T,k); %Demand Backlog constraint in param.SALES equation.
...
for i = 1:nz
    for j = (nt0+1):nt %INITIALIZE AT j=nt0 !!!!
        for k = 1:nloc
            ...
            if (j>param.R_z(i)) %if product can come out of production at this time-step
                q(J(2,i,(j-param.R_z(i,k)),k,1)) = 1; %product i @ location k coming out of production at time j, which started at time-step j - param.R_z(i,k) by definition
            end
            for I = 1:nz
...
%% Inventory/Holding: Sum_i y(i,j,k) <= param.c_y(k) forall j,k
...
for i = 1:nz
    for j = 1:nt
        for k = 1:nloc
            q = zeros(1,dimTotal);
            for T =1:j
                q(J(4,i,T,k,1))=param.mu_w(i,k)^(j-T); %w(i,j,k)
                bSales(count) = bSales(count) + param.mu_w(i,k)^(j-T)*dem(i,T,k); %param.mu_w(i,k)*dem(i,T,k)
            end
            qSales(count,:) = q;
            %bProd(count) = 0;
            count = count+1;
...
oldSol.oldParam.param.Gbom = param.Gbom;
