function [net,extraTS,extraTimes,xlTimes,realTS,xlSkus,trainForesight] = forecast()
%% Importing Data
...
%NN Constants
nTest =1; %=max(trainForesight);
nTrain = floor((maxTrain-delay_constant)); %Number of training samples %Can't train on the testing time-step!
...
trainSamples = ((delay_constant+1):(maxTrain))';%All possible trainable samples
...
testSamples = nt;%=((maxTrain+1):nt)'; %all un-trainable samples CAN be test samples with no sacrifice to training.
...
netOutput = sim(net,trainData); %testDataStrip
...
predIndices = net.divideParam.testInd; %If all data included in one matrix
...
save(['forecast_workspace_',date,'.mat'])
skus_to_plot = [1];
plotDemand(skus_to_plot,realTS,realPreds,xlTimes,realSkus);
